#Standard ellipsoid model using individual priors on terms within the covariance matrix, Σ

#Panel B1: JAGS model code using individual priors on terms of Σ (instead of the inverse-Wishart prior presented in the main text).  We used a Cholesky decomposition to ensure that Σ was positive definite. 
...
model{
  ...
  for (k in 1:ngroups){
    ...
    A[1,1,k] ~ dgamma(0.1,0.1)
    A[2,2,k] ~ dgamma(0.1,0.1)
    A[3,3,k] ~ dgamma(0.1,0.1)
    
    A[2,1,k] ~ dnorm(0, 0.1)
    A[3,1,k] ~ dnorm(0, 0.1)
    A[3,2,k] ~ dnorm(0, 0.1)
    
    cov[1:3,1:3,k] <- A[,,k]%*%t(A[,,k])
    prec[1:3,1:3,k]<-inverse(cov[,,k])
  }
  #likelihood
  for (k in 1:ngroups){
    for(i in 1:maxn){
...
#This appendix includes the R and JAGS code used to run the simulation study (Panel A1) and additional simulation study results (Figure A1).

#Panel A1. The R and JAGS code used to simulate CL and SEV for four different populations with various true CL and SEV values. We assigned each population a true CL and SEV values. The true SEV value is then used to calculate a semi-random covariance matrix, Σ, with the function “getcov(SEV)”. The true CL was used as the mean vector, µ, for each ellipsoid. For illustrative purposes, the code below generates 20 observations for each population from a multivariate normal distribution with the mean vector µ and the simulated covariance matrix Σ. SEV was calculated as a derived quantity based on posterior estimates of Σ after the JAGS model run was completed in R. The code used for hypothesis testing for differences in CL, SEV, and distance between centroids is also included. 
...
overlap<-function(mu1, mu2, sigma1, sigma2){
  axises<-sqrt(eigen(sigma1)$values)
...
# create an array with the isotope names, the group names and the dimnames

y<-array(NA,dim=c(maxn,3,ngroups), dimnames = list(c(1:20), c(isotope_names), c(groupnames)))
...
peng <- tibble(species = fish1$species,
                      d15n = na.approx(fish1$d15n),
                      d13c =  na.approx(fish1$d13c),
                      d34s =  na.approx(fish1$d34s))
...
rgl.bringtotop()
names(colors) <- unique(peng$species)
names(shapes) <- unique(peng$species)

plot3d(peng[,-1], type = "n")
pch3d(peng[,-1],
      pch = shapes[peng$species],
      col = adjustcolor(colors[peng$species], alpha=0.4),
      cex = 0.1,
      decorate = FALSE)

offset <- 0.01
for (sp in unique(peng$species)) {
  xyz <- subset(peng, species == sp)[, 2:4]
  
  # ellipsoids
  mu <- apply(xyz, 2, mean)
  sigma <- cov(xyz)
  ell <- ellipse3d(sigma, centre = mu, level = 0.4)
  ...
  texts3d(max, adj = 0, text = sp, color = colors[sp], cex = 1)
}
...
# Export plot into a interactive HTML file
...
filename <- tempfile(fileext = "isotopic_niche_hybrid_fin_blue.html")


# To save to a file:
# Unload packages that depend on htmlwidgets
...
saveWidget(rglwidget(x = scene3d(), width = figWidth(), height = figHeight()), filename
)
