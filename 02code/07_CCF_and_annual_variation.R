...
# ggsave("/Users/marcruizisagales/Documents/GitHub/Blue-fin-hybrids-isotopes/03figures/CCF.svg", tot,
#        dpi = 300,  width = 22, height = 12, units = "cm")

 # Intraannual variation (d15n, d13c and d34s per yearday and whale)

df$Whale <- factor(df$Whale, levels = c("Bmus2", "Bmus1", "Hyb1", "Hyb2", "Hyb3","Bphy1", "Bphy2", "Bphy3", "Bphy4", "Bphy5"))
...
generate_variants <- function(base_color, n) {
  colorspace::lighten(base_color, amount = seq(0, 0.4, length.out = n))  # up to 40% lighter
}

# assign colors per Whale by varying the species base
individual_colors <- whale_species %>%
  dplyr::group_by(species) %>%
  dplyr::mutate(color = generate_variants(species_colors[species[1]], n())) %>%
  dplyr::ungroup() %>%
  dplyr::select(Whale, color) %>%
  deframe()
...
# ggsave("/Users/marcruizisagales/Documents/GitHub/Blue-fin-hybrids-isotopes/03figures/intraannual_variation.svg", tot1,
